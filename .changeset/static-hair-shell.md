...
to the room the held silhouette actually has. `hair_back` goes static on the
...
playground hero the eye pair lands at -0.21 head half-widths at -30°, where the
rig that shipped before clamped at -0.10 and the reference measures -0.22. Two
...
for more than the plate allows is. And `TurnTargetError`'s `attainable` interval
for `silhouetteRatio` now names a SAFE lower bound rather than the narrowest
ratio the layer set could hold: the ratio sizes the slide as well, so a narrower
ratio arrives with a smaller slide and can still hold — everything at or above
the bound holds, and some below it do too.
